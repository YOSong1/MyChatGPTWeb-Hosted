...
### 4.5 도구 기능 (2026-09-10 추가)
- 호출 계층을 Chat Completions에서 **Responses API**로 바꿨다. 웹 검색과 코드 실행 도구는 Responses API에서만 쓸 수 있다. 저장 형식은 그대로다.
- **🌐 웹 검색**: 입력창 위 스위치. 켜면 요청에 `web_search` 도구를 붙인다. 답변의 `url_citation` annotation을 모아 "출처" 접이식 목록으로 보여준다. 검색 전용 모델(gpt-4o-search-preview 계열)은 폐기되어 쓰지 않는다.
- **📎 파일 생성**: 스위치를 켜면 `code_interpreter` 도구를 붙인다. 모델이 OpenAI 샌드박스에서 파이썬으로 파일을 만들면 `container_file_citation` annotation에서 container_id와 file_id를 얻어 즉시 내려받아 `<데이터 폴더>/files/<대화 id>/`에 저장하고, 답변 아래 다운로드 버튼으로 보여준다. 모델이 링크를 안 적어 annotation이 없을 때를 대비해 컨테이너 파일 목록을 직접 조회하는 보완 경로가 있다. 컨테이너는 20분 뒤 사라지므로 응답 직후에 받아야 한다.
- 답변 속 `sandbox:/mnt/data/...` 링크는 앱 밖에서 열리지 않으므로 굵은 글씨 이름으로 바꿔 저장한다.
- 코드 블록(3줄 이상)에는 언어에 맞는 확장자로 **💾 snippet-N.ext** 저장 버튼을 붙인다. 도구 없이 동작한다.
- 두 스위치 상태는 `config.json`에 저장된다. 둘 다 꺼져 있으면 도구 없이 호출해 기존과 같은 비용이다.
- 비용: 검색 1회, 코드 실행 세션 1회당 소액이 추가된다. 학생 안내문에 "평소에는 꺼 두라"고 적었다.
